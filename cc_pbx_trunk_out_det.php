...
$fmenu_link_back = "cc_pbx_trunk_out_list";
...
$tactext		= get_param("tactext");
$callertext		= get_param("callertext");
$prefixtext		= get_param("prefixtext");
$begintext		= get_param("begintext");
$timeouttext	= get_param("timeouttext");
$host			= get_param("host");
$qos_type		= get_param("qos_type");
$status			= get_param("status");

if($iddet!=''){
	$sqlv = "SELECT * 
		FROM cc_pbx_trunk_out a 
                WHERE a.id > 0
                      AND a.id='$iddet'";
	...
	if($recv = mysqli_fetch_array($resv)){
		...
		$host		    	= $recv['trunk_id'];
		$tactext		    = $recv['tac'];
		$callertext		    = $recv['caller_id'];
		$prefixtext		    = $recv['prefix'];
		$begintext		    = $recv['beg_digit'];
		$timeouttext		= $recv['timeout'];
		$qos_type		    = $recv['qos_type'];
		$status				= $recv['status'];

	}
}
//file save data
$save_form = "view/cc/cc_pbx_trunk_out_save.php";

//url to get new data sip trunk
$get_sip_trunk_url="view/cc/get_sip_trunk.php";
				...
				<div style="margin:10px 10px 10px 10px;">
					<div>
							...
							<div class="form-body">		
	                    		...
	                    		$txtlabel[$x]      = "Context Name";
	                    		$bodycontent[$x]   = input_text_temp("context","context",$context,"","required","form-control border-primary");
	                    		$x++;
	                    		
	                    		$txtlabel[$x]      = "Sip Provider";
	                    		$bodycontent[$x]   = cc_get_select_host_by_server_id($host,$condb,$serverid);
	                    		$x++;

	                    		$txtlabel[$x]      = "Trunk Access Code";
	                    		$bodycontent[$x]   = input_text_temp_keypress("tactext","tactext",$tactext,"","required","form-control border-primary","return validate_number(event)");
	                    		$x++;
	                    		
								$txtlabel[$x]      = "Caller Id";
	                    		$bodycontent[$x]   = input_text_temp_keypress("callertext","callertext",$callertext,"","required","form-control border-primary","return validate_number(event)");
	                    		$x++;

	                    		$txtlabel[$x]      = "Prefix";
	                    		$bodycontent[$x]   = input_text_temp("prefixtext","prefixtext",$prefixtext,"","required","form-control border-primary");
	                    		$x++;

	                    		$txtlabel[$x]      = "Begin Digit";
	                    		$bodycontent[$x]   = input_text_temp_keypress("begintext","begintext",$begintext,"","required","form-control border-primary","return validate_number(event)");
	                    		$x++;

	                    		$txtlabel[$x]      = "Time (sec)";
	                    		$bodycontent[$x]   = input_text_temp_keypress("timeouttext","timeouttext",$timeouttext,"","required","form-control border-primary","return validate_number(event)");
	                    		$x++;

	                    		$txtlabel[$x]      = "Qos Type";
	                    		$bodycontent[$x]   = get_select_qos_type($qos_type,$condb);
	                    		$x++;
...
<!--   Core JS Files   -->
	...
	$('#server_id').on("change", function(e) { 
		get_sip_trunk_server();
		
	});

	function get_sip_trunk_server(){

		let server_id = $('#server_id').find(':selected').val();
		
		let host = "<?php echo $host; ?>";
		let url = "view/cc/get_sip_trunk.php";
		$('#host_id').select2('destroy');
		$('#host_id').empty();
		$.ajax({
			type:"POST",
			url:url,
			data:{server:server_id,host:host},
